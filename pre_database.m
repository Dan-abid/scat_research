...
function db = pre_database(src,feature_fun,opt)
if nargin < 3
    opt = struct();
end
...
opt = fill_struct(opt, 'parallel',0);
...
if opt.parallel
    ...
    for j=1:c
        for k = 1:b
            count=count+1;
            file_objects = find([src.objects.ind]==count);
            ...
            buf = apply_features(src.files(:,count),src.objects(file_objects),feature_fun,opt);
