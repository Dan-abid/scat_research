...
% 			x = data_read(src.files{k});
		...
		for k = 1:length(src.files)
			...
			tm0 = tic;
			% Load the complete file and normalize as needed.
% 			x = data_read(src.files{k});%hao the defaul way reading data
            start=randi([1,20000],1);%hao reading data from here
            if k<251
                x=opennoleak(start:start+Global_N-1,ind).';%hao
            else
                x=openleak(start:start+Global_N-1,ind).';%hao   
            end     
